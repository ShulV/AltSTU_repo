...
public class TileCut {
   int augment(int[][] g, int[][] now, int at, int sink, boolean[] seen, int sofar) {
      if (at == sink) return sofar;
      ...
      for (int i=0; i<g.length; i++)
         if (now[at][i] < g[at][i]) {
            int t = augment(g, now, i, sink, seen, Math.min(sofar, g[at][i]-now[at][i])) ;
            if (t > 0) {
               now[at][i] += t ;
               now[i][at] -= t ;
               return t ;
   ...
   int[][] flow(int[][] g, int src, int sink) {
      ...
      while (augment(g, now, src, sink, new boolean[g.length], 1000000000) > 0);
      return now;
   }
