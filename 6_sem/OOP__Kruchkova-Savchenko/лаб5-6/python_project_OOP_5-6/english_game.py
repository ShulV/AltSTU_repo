...
import csv
from wrong_answer_factory import WrongAnswerFactory
...
class EnglishGame(Game):
    ...
    def __init__(self):
        super().__init__()
        self.__filename = ENGLISH_WORDS_FILE
    ...
    def show_ask(self):
        """показ слова """
        words = self.get_word_and_translation(self._counter)
        if words is None:
            print('Слова кончились!')
            return False
        english_word = words[0]
        self._correct_answer = words[1]
    ...
    def check_correct_answer(self):
        ...
        if super().check_correct_answer():
            ...
            WrongAnswerFactory().get_wrong_answer(self._user_answer)
            print(WrongAnswerFactory().wrong_answers_counter)
            print(WrongAnswerFactory().wrong_answers)
            return False
