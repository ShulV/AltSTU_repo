...
class Graph:
    ...
    def add_new_event_in_events_graph(self, event):
    ...
    def clear_graph_params(self):
        """ очистка параметров графа, которые будут пересчитаны заново """
        # очистка посещенности вершин
        n = len(self.graph_table['arc_start'])
        for i in range(n):
            self.graph_table['is_visited'][i] = None
        # очистка параметров графа
        self.first_top = None  # первая вершина графа
        self.last_top = None  # последняя вершина графа
        self.struct_graph_works_num = 0  # количество работ упорядоченного графа
        # очистка таблицы структированного графа для повторного заполнения
        self.struct_graph_table['arc_start'] = []
        self.struct_graph_table['arc_end'] = []
        self.struct_graph_table['weight'] = []
        self.struct_graph_table['is_visited'] = []
        self.struct_graph_table['full_time_reserve'] = []
        self.struct_graph_table['independent_time_reserve'] = []
        # очистка таблицы событий для повторного заполнения
        self.graph_events['event'] = []
        self.graph_events['early_term'] = []
        self.graph_events['late_term'] = []
        self.graph_events['reserve_time'] = []
        self.graph_events['in_works'] = []
        self.graph_events['out_works'] = []
        self.graph_events['layer'] = []
        self.graph_events['visited'] = []
    ...
    def find_all_vertices(self):
        ...
        self.add_new_event_in_events_graph(self.first_top)
        ...
        for _i in range(0, self.works_num):
            if self.graph_table['arc_start'][_i] == self.first_top:
                ...
                if self.graph_table['arc_end'][_i] not in self.graph_events['event']:
                    self.add_new_event_in_events_graph(self.graph_table['arc_end'][_i])
        ...
        for i in range(0, self.works_num):
            if self.graph_table['arc_start'][i] == self.current_way[len(self.current_way) - 1]:
                ...
                if self.graph_table['arc_end'][i] not in self.graph_events['event']:
                    self.add_new_event_in_events_graph(self.graph_table['arc_end'][i])
                self.recursive_find_all_vertices()
    ...
    def copy_row_to_structed_graph_table(self, index):
    ...
    def copy_graph_from_structed_graph(self):
        """ копирует данные структурированного графа в неструктурированный для дальнейшей работы с первым """
        self.graph_table['arc_start'] = self.struct_graph_table['arc_start']
        self.graph_table['arc_end'] = self.struct_graph_table['arc_end']
        self.graph_table['weight'] = self.struct_graph_table['weight']
        self.graph_table['is_visited'] = self.struct_graph_table['is_visited']


    def delete_row_from_graph_table(self, index):
        """ удалить строку неструктрированного графа по индексу """
        start = self.graph_table['arc_start'].pop(index)
        end = self.graph_table['arc_end'].pop(index)
        weight = self.graph_table['weight'].pop(index)
        print(f'Работа ({start}, {end}) [{weight}] удалена')
        self.graph_table['is_visited'].pop(index)
        self.works_num -= 1

    def change_work_time(self, index, time):
        """ изменить время работы по индексу"""
        if index < self.struct_graph_works_num:
            self.struct_graph_table['weight'][index] = time
            return True
        return False
    ...
    def print_row(self, row_index, sorted_graph=False):
        ...
        if sorted_graph:
            print('|{index:^4}|{start:^10}|{end:^10}|{weight:^25}|{full_res:^25}|{ind_res:^25}|'.format(
                index = row_index,
            ...
            print('|{index:^4}|{start:^10}|{end:^10}|{weight:^10}|'.format(
                index=row_index,
    ...
    def print_graph(self, sorted_graph=False):
        ...
        if sorted_graph:
            print('|{num:^4}|{start:^10}|{end:^10}|{weight:^25}|{full_res:^25}|{ind_res:^25}|'.format(
                num='№',
                start='A',
                end='B',
            ...
            print('|{num:^4}|{start:^10}|{end:^10}|{weight:^10}|'.format(
                num='№',
                start='A',
                end='B',
                weight='Вес',
        ...
        while self.works_num != self.struct_graph_works_num:
            for i in range(0, self.works_num):
                if vertex_queue[0] == self.graph_table['arc_start'][i] and not self.graph_table['is_visited'][i]:
                    self.copy_row_to_structed_graph_table(i)
    ...
    def print_gant_diagram(self):
        """ вывод диграммы Ганта """
        max_console_size = 131
        step = 10
        print('\nВывод диаграммы Ганта')
        cprint('{day:^20}'.format(day='День'), on_color='on_white', end='')
        for day_num in range(0, max_console_size, step):
            if day_num % 20 == 0:
                cprint('{day_num:<10}'.format(day_num=day_num), on_color='on_green', end='')
            else:
                cprint('{day_num:<10}'.format(day_num=day_num), on_color='on_white', end='')
        print()
        n = len(self.struct_graph_table['arc_start'])
        for i in range(n):
            ...
            if i % 2 == 0:
                ...
                cprint(' ' * weight, 'green', 'on_blue')


def main():
    in_file = "input.csv"
    graph = Graph()
    graph.read_from_file(in_file)
    loop_counter = 0
    while True:
        if loop_counter > 0:
            graph.clear_graph_params()
        graph.print_graph()
        ...
        # зануление перепроверяемых параметров (для циклического запуска функций на повтор)
        ...
        # выводим диаграмму Ганта
        graph.print_gant_diagram()
        # копируем структурированный граф в неструктурированный, чтобы производить добавление/удаление/изменение работ
        ...
        while loop_again:
            ...
            if choice == '1':
                ...
                end_vertex = int(input('Введите шифр конечной вершины (например: 149):\t'))
                weight = int(input('Введите продолжитлеьность работы (например: 15):\t'))
                graph.add_row_in_graph_table(start_vertex, end_vertex, weight)
                print('Работа добавлена')
            elif choice == '2':
                print('Вы выбрали "удалить работу"')
                graph.print_graph()
                index = int(input('Введите номер работы, которую удалить:\t'))
                graph.delete_row_from_graph_table(index)
                print('Работа удалена')
                graph.print_graph()
            elif choice == '3':
                print('Вы выбрали "изменить время работы"')
                graph.print_graph()
                index = int(input('Введите номер работы, время которой вы хотите изменить:\t'))
                time = int(input('Введите время работы:\t'))
                if graph.change_work_time(index, time):
                    print('Время работы изменено')
                    graph.print_graph()
                else:
